...
con = mysql.connector.connect(
	host="localhost", user="root", password="@477447Vs", database="emp")
...
def check_employee(employee_id):
	...
	# rom employee Table
	sql = 'select * from emp_rec where id=%s'
	...
	if r == 1:
		...
		return False# Function to mAdd_Employee
...
def Add_Employ():
	...
	if(check_employee(Id) == True):
		...
		# the Employee Table
		sql = 'insert into emp_rec values(%s,%s,%s,%s)'
		c = con.cursor()
...
def Promote_Employee():
	...
	if(check_employee(Id) == False):
		...
		# with given Id
		sql = 'select salary from emp_rec where ID=%s'
		data = (Id,)
		c = con.cursor()
		...
		# given Id
		sql = 'update emp_rec set salary=%s where ID=%s'
		d = (t, Id)
...
def Remove_Employ():
	...
	if(check_employee(Id) == False):
		...
		sql = 'delete from emp_rec where ID=%s'
		data = (Id,)
		c = con.cursor()
...
def menu():
	print(colored('__________Welcome to Employee Management Record___________', 'green', attrs=['bold']))
	print("Press ")
